...
class Buffer:
    def __init__(self, trajectory_size=4000):
        self.trajectory_size = trajectory_size
        self.users = dict()

    def store(self, sample):
        users, memory, action, reward, next_user, next_memory = sample
        for num in range(len(users)):
            self.users[users[num]] = self.users.get(users[num], []) + [[memory[num, :],
                                                                        action, reward[num], next_user[num],
                                                                        next_memory[num, :]]]

    def sample(self, user):

        return np.vstack(self.users[user])
