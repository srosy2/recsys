...
class PMF(object):
    ...
    def fit(self, train_vec, test_vec):
        ...
        num_item = int(max(np.amax(train_vec[:, 1]), np.amax(test_vec[:, 1]))) + 2  # 第1列，movie总数
    ...
    def predict(self, invID):
        return np.dot(self.w_Item, self.w_User[int(invID), :])  # numpy.dot 点乘
    ...
    def set_params(self, parameters):
        if isinstance(parameters, dict):
            ...
            self.num_batches = parameters.get("num_batches", 100)
